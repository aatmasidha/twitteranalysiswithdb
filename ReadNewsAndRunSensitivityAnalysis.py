...
def readJSONFileByItems(fileName):
    try:
                        ...
                        for newsKey, newsValue in newsDetailsValue.items():
                            if newsKey == 'id':
                                ...
                                if(urls != None and  len(urls) > 0):
                                    ...
                                    logger.debug(getAnalysis(ProcessText.getPolarity(articleString)))  
                                    values['polarity'] = ProcessText.getPolarity(articleString)      
                                    newsDetailsAndSensitivity = values
                                    
                                    
                                record['articleText'] = newsDetailsAndSensitivity['articleText'] 
                                record['score'] = newsDetailsAndSensitivity['score']
                                record['classifier'] = newsDetailsAndSensitivity['classifier']
                                record['emotion'] = newsDetailsAndSensitivity['emotion']
                                record['polarity'] = newsDetailsAndSensitivity['polarity']
                                record['subjectivity'] = newsDetailsAndSensitivity['subjectivity']
                                ...
                                # reTweetLocationCount = {}
                                # for reTweetKey, reTweetValue in newsValue.items():
                                #     for reTweetUserKey, reTweetUserValue in  reTweetValue.items(): 
                                #             if 'reTweetLocation' == reTweetUserKey and len(reTweetUserValue) > 0:
                                #                 if reTweetUserValue in reTweetLocationCount[reTweetUserValue]: 
                                #                         reTweetLocationCount[reTweetUserValue] = 1
                                #                 else:
                                #                         reTweetLocationCount[reTweetUserValue] = reTweetLocationCount[reTweetKey] + 1
        ...
        with open( outputPath + "/" + newsAgencyName + "_" +"output" + "_" + dateStr + "_T_" + timeStr + ".json", 'w', encoding='utf8') as json_file:
